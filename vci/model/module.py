...
class CompoundEmbedding(nn.Module):
    def __init__(self, num_embeddings: int, embedding_dim: int) -> None:
        ...
        self.pad = nn.Parameter(torch.empty((1, embedding_dim)), requires_grad=False)
        self.reset_parameters()

    def reset_parameters(self) -> None:
        nn.init.normal_(self.weight)
        nn.init.zeros_(self.pad)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        # input: batch_size x num_index
        weight = torch.cat([self.weight, self.pad], dim=0) # allow -1 indexing
